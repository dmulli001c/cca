...
namespace fastJSON
{
    internal sealed class JSONSerializer
    {
		...
		// New class named TypeValue added to store the 
		// Type and index refrance to be able to get the 
		// member function from the Type object.
        public class TypeValue
        {
            public Type type;
            public int index;
        }
        private Dictionary<string, TypeValue> _globalTypes = new Dictionary<string, TypeValue>();
        ...
        internal string ConvertToJSON(object obj)
        {
            ...
            if (_params.UsingGlobalTypes && _globalTypes != null && _globalTypes.Count > 0)
            {
                ...
                var Appendfunctions = false;

                //I added the following code to be able to serialize the public functions:
                sb.Append("\"functions\":[");

                foreach (var kv in _globalTypes)
                {
                    Type objectType = kv.Value.type;

                    if (objectType == typeof(UnityEngine.MonoBehaviour) || objectType.BaseType == typeof(UnityEngine.MonoBehaviour))
                    {
                        if (pendingSeparator)
                            sb.Append(',');
                        else
                        ...
                        sb.Append("{\"$type\": \"");
                        sb.Append(kv.Value.index);
                        sb.Append("\",\"PublicMethods\": [");

                        bool pendingSeparatorLevel2 = false;
                        foreach (MethodInfo mth in kv.Value.type.GetMethods(BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public))
                        {
                            Appendfunctions = true;

                            if (pendingSeparatorLevel2)
                                sb.Append(",");
                            else
                                pendingSeparatorLevel2 = true;

                            // Display name and type of the concerned member.
                            sb.Append("{\"" + mth.Name + "\":{\"arguments\":[");
                            bool pendingSeparatorLevel3 = false;
                            foreach (var param in mth.GetParameters())
                            {
                                if (pendingSeparatorLevel3)
                                    sb.Append(",");
                                else
                                    pendingSeparatorLevel3 = true;

                                sb.Append("{\"name\":\"" + param.Name + "\",\"type\":\"" + param.ParameterType + "\"}");

                            }
                            sb.Append("],\"returns\": \"" + mth.ReturnParameter.ParameterType + "\"}}");
                        }
                        sb.Append("]}");
                    }
                }
        ...
        private void WriteValue(object obj)
        {
            ...
            else if (obj is UnityEngine.Texture || obj is UnityEngine.Font|| obj is UnityEngine.AudioClip|| obj is UnityEngine.TextAsset
                || obj is UnityEngine.Mesh || obj is UnityEngine.AnimationClip)
            {
                WriteAssetReferenceObject(obj);
            }
        ...
        private void WriteAssetReferenceObject(object obj)
        {
            int i = 0;
            ...
            if (_params.UseExtensions)
            {
                ...
                else
                {
                    ...
                    {
                        dt = new TypeValue();
                        dt.index = _globalTypes.Count + 1;
                        dt.type = t; 
                        _globalTypes.Add(ct, dt);
                    }
                    WritePairFast("$type", dt.index.ToString());
                }
                append = true;
            }
            ...
            WritePair("name", (obj as UnityEngine.Object).name);
        ...
        private void WriteObject(object obj)
        {
            ...
            //   I added the following code to be able to serialize the GameObject Components and the GameObject children
            ...
            if (typeof(GameObject) == obj.GetType())
            {
                ...
                if (components != null)
                {
                    ...
                    if (_params.SerializeToLowerCaseNames)
                        WritePair("components", components);
                    else
                        WritePair("Components", components);
                    append = true;
                }
                ...
                {
                    for (int index = 0; index < childrenTransforms.Length; index++)
					{
                        // make sure that is the correct child to the current parent.
                        if (childrenTransforms[index].parent == gameObject.transform)
                            childrenGameObjects.Add(childrenTransforms[index].gameObject);
					}
                    ...
                    if (childrenGameObjects.Count > 0)
                    {
                        ...
                        if (_params.SerializeToLowerCaseNames)
                            WritePair("children", childrenGameObjects);
                        else
                            WritePair("Children", childrenGameObjects);
                        append = true;
                    }
